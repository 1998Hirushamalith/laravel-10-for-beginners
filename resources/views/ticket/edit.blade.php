            ...
            <form method="POST" action="{{ route('ticket.update',$ticket->id) }}" enctype="multipart/form-data">
                ...
                <div class="mt-4">
                    @if ($ticket->attachement)
                        <a href="{{ '/storage/' . $ticket->attachement }}" target="_blank" class="text-black" >See attachment</a>
                    @endif
                    <x-input-label for="attachement" :value="__('Attachment (if any)')" />
                    <x-file-input name="attachement" id="attachement" />
